...
public function getActivities( $type = null, $param = null, $param2 = null, $page = 1 )
	...
	public function getConditon($type = null, $param = null, $param2 = null)
	{
        ...
        //region conditions
        if($viewer['User']['job_belong_to'] == 'pharmacy'){
            $region = $viewer['User']['kpasame'];
        }elseif($viewer['User']['com_department'] == 'sales' && !empty($viewer['User']['sale_area'] && !in_array($viewer['User']['job_belong_to'], array('pharmacy_chain', 'it_software')))){
            $regions = explode(',',$viewer['User']['sale_area']);
            if(count($regions) == 1) {
                $region = $viewer['User']['sale_area'];
            }else{
                $or = array();
                foreach ($regions as $region){
                    $or[] = array(
                        'Activity.region LIKE' => '%'.$region.'%',
                    );
                }
            }
        }else{
            $region = $viewer['User']['sidocode'];
        }

        if(!empty($or)){
            $region_cond = array(
                'Activity.privacy' => MY_REGION,
                'OR' => $or,
            );
        ...
        $job_cond = array(
            'Activity.privacy' => MY_JOB_AREA,
            'Activity.job_area' => $viewer['User']['job_belong_to'],
        );
		...
		switch ($type) {
                    ...
                    if ($viewer['Role']['is_admin']) {
                        $cond = array(
                            'OR' => array(
                                ...
                                array('Activity.action' => "friend_add", 'NOT' => array('Activity.privacy' => array(PRIVACY_ME, MY_REGION, MY_JOB_AREA)))
                            )
                        );
                    } else {
                        $cond = array(
                            'OR' => array(
                                ...
                                $this->addBlockConditionTarget(array('Activity.type' => APP_USER, 'Activity.user_id' => $friends, 'NOT' => array('Activity.privacy' => array(PRIVACY_ME, MY_REGION, MY_JOB_AREA)))),
                                $this->addBlockConditionTarget($region_cond),
                                $this->addBlockConditionTarget($job_cond),
                            )
                        );
                    }
                ...
                else
                {
                    ...
                    $cond = array(
                        'OR' => array(
                            ...
                            $this->addBlockConditionTarget(array ('Activity.user_id' => $param,'Activity.action' => "friend_add", 'NOT' => array('Activity.privacy' => array(PRIVACY_ME, MY_REGION, MY_JOB_AREA)))),
                            $this->addBlockConditionTarget($region_cond),
                            $this->addBlockConditionTarget($job_cond),
                        ),
			...
			case 'friends':
                ...
                $region_cond['Activity.user_id'] = $friends;
                $job_cond['Activity.user_id'] = $friends;
