...
class User extends AppModel
{
    ...
    public $actsAs = array(
        'MooUpload.Upload' => array(
            ...
            'default_avatar' => array(
                'path' => '{ROOT}webroot{DS}uploads{DS}users{DS}avatar{DS}',
                'thumbnailSizes' => array(
                    'size' => array('50_square','100_square','200_square', '600')
                ),
            ),
	...
	public $validate = array(  
			...
			'email' => 	array( 	 
                ...
                'checkMailUnique' => array(
                    'rule' => 'checkMailUnique',
			...
			'password' => array( 
				...
				'allowEmpty' => false,
				'message' => 'Password must have at least 8 characters and should contain one digit and one special character.'
			),
			'password2' => array( 
        ...
        'job_belong_to' => 	array(
            'rule' => 'notBlank',
            'message' => 'Job belong to is required'
        ),
        'university_id' => 	array(
            'rule' => 'notBlank',
        ...
        'admission_year' => 	array(
            ...
            'number' => array(
                'rule' => 'numeric',
                'message' => 'Admission year must only contain numbers'
            ),
            'length' => array(
                'rule'    => array('between', 4,4),
                'message' => 'Admission year must be 4 characters long.'
            ),
            'validYear' => array(
                'rule' => 'validYear',
                'message' => 'Admission year must be from 1930 to the current year'
            )
        ),
        'mobile' => 	array(
            'required' => array(
            ...
            'numeric' => array(
                'rule' => 'numeric',
                'message' => 'Mobile must only contain numbers'
            ),
//            'length' => array(
...
//            ),
        ...
        'sub_mail' => 	array(
            'email' => array(
                'rule' => 'email',
                'allowEmpty' => true,
                'message' => 'Sub email must be a valid email'
            ),
            'checkSubUnique' => array(
                'rule' => 'checkSubUnique',
                'message' => 'Email already exists'
            )
        ),
        ...
        'com_name' => 	array(
            'rule' => 'notBlank',
            'message' => 'Company(Pharmacy) is required'
        ),
        'com_zip' => 	array(
            'required' => array(
                'rule' => 'notBlank',
        ...
        'com_department' => 	array(
            'rule' => 'notBlank',
            'message' => 'Department is required'
        ),
        'sale_area' => 	array(
            'rule' => 'notBlank',
            'message' => 'Sale area is required'
        ),
        'represent_email' => 	array(
            'email' => array(
                'rule' => 'email',
                'allowEmpty' => true,
                'message' => 'Represent email must be a valid email'
            ),
        )
	);

    public function checkSubUnique($values) {
        if (!empty($values['sub_mail']))
        {
            $viewer = MooCore::getInstance()->getViewer(true);
            $count = $this->find('count', array(
                'conditions' => array('OR' => array('User.email' => $values['sub_mail'], 'AND' => array('User.sub_mail' => $values['sub_mail'], 'User.id <>' => $viewer)))
            ));
    ...
    public function checkMailUnique($values) {
        if (!empty($values['email']))
        {
            $viewer = MooCore::getInstance()->getViewer(true);
            $count = $this->find('count', array(
                'conditions' => array('OR' => array('User.sub_mail' => $values['email'], 'AND' => array('User.email' => $values['email'], 'User.id <>' => $viewer)))
            ));
    ...
    public function validYear($values)
    {
        if (!empty($values['admission_year'])) {
            if($values['admission_year'] <1930 || $values['admission_year'] > date('Y')){
                return false;
            }
        }
        return true;
    ...
    public function limitDuplicates($password, $limit) {
        $password = $password['password'];
        $match    = preg_match('/^(?=.*\d)(?=.*[^a-zA-Z0-9]).{8,}$/m', $password);
        if($match) {
            return true;
        }
        return false;
