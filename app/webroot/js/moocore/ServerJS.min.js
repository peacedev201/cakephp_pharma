...
(function (root, factory) {
    if (typeof define === 'function' && define.amd) { 
        // AMD
        define(['jquery', 'mooOverlay', 'mooBehavior','mooPhrase', 'mooResponsive', 'mooShare', 'mooNotification', 'mooPhoto', 'mooUser',
...
}(this, function ($, mooOverlay, mooBehavior,mooPhrase, mooResponsive, mooShare, mooNotification, mooPhoto, mooUser) {
    ...
    var init = function () {
        ...
        function showSpecialityAlert(){
            ...
            msg = mooPhrase.__("sorry")+".<br><br>"+mooPhrase.__("menu_pharmacists")+".<br>"+mooPhrase.__("please_use_below")+"<b>“"+mooPhrase.__("search")+"”</b>"+mooPhrase.__("menu1");
            // text = 'close';
            // Set title
        ...
        function showNonComDepartmentAlert(){
            message = mooPhrase.__('information');
            msg = mooPhrase.__("sorry")+".<br><br>"+mooPhrase.__("menu_pharmacy")+".<br>"+mooPhrase.__("please_use_below")+"<b>“"+mooPhrase.__("search")+"”</b>"+mooPhrase.__("menu1");
            // text = 'close';
            // Set title
            $($('#portlet-config  .modal-header .modal-title')[0]).html(message);
            // Set content
        ...
        function showDepartmentSalesAlert(){
            message = mooPhrase.__('information');
            msg = mooPhrase.__("dear_sales_reps")+".<br><br>"+mooPhrase.__("you_have_reached_the_right_place")+".<br>"+mooPhrase.__("But_your_sales_area_seems_to_be")+".";
            msg += mooPhrase.__("plz_click_this")+" ";
            msg += '<a href="'+mooPhrase.__("cprofile_link")+'">'+mooPhrase.__("slink")+'</a> ';
            msg += mooPhrase.__("and_define_your_sales_area_at_below_fields")+"..<br><br>";
            msg += mooPhrase.__("department")+'&nbsp&nbsp&nbsp';
            msg += '<input type="radio"> '+mooPhrase.__("sales")+'&nbsp';
            msg += '<input type="radio"> '+mooPhrase.__("marketing")+'&nbsp';
            msg += '<input type="radio"> '+mooPhrase.__("others")+'<br><br>';
            msg +=  mooPhrase.__("sales_area")+'&nbsp&nbsp&nbsp<input type="text">';
            // text = 'close';
            // Set title
            $($('#portlet-config  .modal-header .modal-title')[0]).html(message);
            // Set content
        ...
        $('#browse a:not(.overlay):not(.no-ajax)').click(function () {
            if($(this).parent().hasClass('non-specialty')){
                showSpecialityAlert();
                return false;
            }
            if($(this).parent().hasClass('non-sales_com_department')){
                showDepartmentSalesAlert();
                return false;
            }
            if($(this).parent().hasClass('non-com_department')){
                showNonComDepartmentAlert();
                return false;
            }
            ...
            if (event.keyCode == '13') {
                if ($(this).val() != '') {
                    ...
                    if ($(this).val().indexOf('#') > -1) {
                        window.location = mooConfig.url.base + '/search/hashtags/' + encodeURIComponent(searchStr);
                    } else {
                        window.location = mooConfig.url.base + '/search/index/' + encodeURIComponent(searchStr);
                    }
                }
            }
        });
