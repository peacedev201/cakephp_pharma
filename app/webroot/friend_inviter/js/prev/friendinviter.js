(function ($) {
    ...
    $('#send_invite').click(function () {
        inviteFriends($('#provide_id').val());
    });

    $('#checkallBox').click(function () {
         $('.check_item').prop('checked', false);
        var totalContacts = $('#total_contacts').val();
        var total_allow_select = $('#total_allow_select').val();
    ...
    $('.check_item').click(function () {
        ...
        if (check_element.is(':checked') && count_contacts >= total_allow_select) {
            ...
            alert('You have exceeded the maximum number of people you can invite');
        }
    ...
    $('#skip_action').click(function () {
        var url = baseUrl + '/friend_inviters';
    ...
    $('.respondRequest').on('click', function () {
        ...
        $.post(baseUrl + '/friends/ajax_respond', {id: data.id, status: data.status}, function (response) {
    ...
    $('#invite_btn').click(function () {
        ...
        disableButton('invite_btn');
        $(".error-message").hide();
        $.post(baseUrl + '/friend_inviters/ajax_invite',$("#invite_form").serialize(), function (data) {
            enableButton('invite_btn');
            ...
            if (json.result == 1)
            {
                ...
                $("#message").val('');
                $(".error-message").hide();
                mooAlert('Your invitation has been sent');
            }
            else
            {
                $(".error-message").show();
...
var inviteFriends = function (socialtype) {
    ...
    if (checked) {
        ...
        var url = baseUrl + '/friend_inviters/invitetosite';
        $.post(url,postData, function (data) {
            $('#show_sitefriend').hide();
            var json = $.parseJSON(data);
            
            if (json.result == 1) {
                window.location.href = baseUrl + '/friend_inviters/invited';
            } else {
...
var tweetWindow = function (url) {
    ...
    window.open("http://twitter.com/share?text=" + encodeURIComponent(text) + "&url=" +
            encodeURIComponent(url) + "&count=none/", "tweet", "height=300,width=550,resizable=1")
}

var faceWindow = function (url) {
    window.open("http://www.facebook.com/sharer.php?u=" + encodeURIComponent(url), "facebook", "height=300,width=550,resizable=1")
}
...
if(typeof qq != "undefined"){
    var uploader = new qq.FineUploader({
        ...
        autoUpload: true,
        text: {
            uploadButton: '<div class="upload-section"><i class="fa fa-file-text-o"></i>Drag or click here to upload a CSV file</div>'
        },
        validation: {
        ...
        request: {
            endpoint: baseUrl + "/friend_inviters/uploads"
        },
        callbacks: {
