...
<?php $this->Html->scriptStart(array('inline' => false, 'domReady' => true, 'requires' => array('jquery', 'mooQuestion'), 'object' => array('$', 'mooQuestion'))); ?>
	...
	$('.pagination a').each(function(){
    	$(this).attr('href',$(this).attr('href')+'&app_no_tab=1');
	});	
    	...
    	<div class="moo_app">	
	        ...
	        <div class="question-detail-action">
	            <div class="list_option">
	            	<?php if ($uid):?>    
		                <div class="dropdown">
		                    <button id="question_edit_<?php echo $question['Question']['id']?>" class="mdl-button mdl-js-button mdl-js-ripple-effect mdl-button--icon">
		                        <i class="material-icons">more_vert</i>
		                    </button>
		                     <ul class="mdl-menu mdl-menu--bottom-right mdl-js-menu mdl-js-ripple-effect" for="question_edit_<?php echo $question['Question']['id']?>">
		                        <?php if ($canEdit):?>
			                        <li><a href="<?php echo $this->request->base?>/questions/create/<?php echo $question['Question']['id']?>?app_no_tab=1" title="<?php echo __d('question','Edit Question')?>"><?php echo __d('question','Edit Question')?></a></li>
		                        ...
		                        <li>
									<li><a href="<?php echo $this->request->base?>/reports/ajax_create/Question_Question/<?php echo $question['Question']['id'];?>" title="<?php echo __d('question','Report Question')?>"><?php echo __d('question','Report Question')?></a></li>
		                        </li>
		                        <?php if ($question['Question']['privacy'] != PRIVACY_ME): ?>
			                        <!-- not allow sharing only me item -->
			                        <?php echo $this->element('share/menu',array('param' => 'Question_Question','action' => 'question_item_detail' ,'id'=>$question['Question']['id'])); ?>
	                        	<?php endif; ?>
		                    </ul>
		                </div>
	               	<?php endif;?>
	        ...
	        <div id="question_content" class="row question-main-content">
				...
				<div class="list_question col-md-11 col-xs-10">
					...
					<div class="row question_info">
						<div class="clear">
							<a href="<?php echo $question['User']['moo_href']?>">
							...
							<?php if ($question['Category']):?>
								<span class="question-category">
									<span class="question-in"><?php echo __d('question','in');?><span></span> <a href="<?php echo $this->base;?>/questions/index?category=<?php echo $question['Category']['id']?>"><?php echo $helper->getCategoryName($question['Category']['id'])?></a>
								</span>                
							<?php endif;?>
						</div>
					</div>
					<div class="comments-container">
						<?php echo $this->element('comments',array('type'=>'Question','id'=>$question['Question']['id'],'comments'=>$comments,'comment_count'=>$question['Question']['comment_count']),array('plugin'=>'Question'));?>
					</div>
				</div>
	        </div>
	        <div class="row answer_menu">
	        	...
	        	<div class="col-md-10">
	        		<ul id="li_question" class="sort-questions">
						<li class="disabled"><a <?php if ($tab == 'active') echo 'class="active"'?> href="<?php echo $question['Question']['moo_href']?>?tab=active&app_no_tab=1"><?php echo __d('question','Active');?></a></li>
						<li class="disabled"><a <?php if ($tab == 'vote') echo 'class="active"'?> href="<?php echo $question['Question']['moo_href']?>?tab=vote&app_no_tab=1"><?php echo __d('question','Votes');?></a></li>
						<li class="disabled"><a <?php if ($tab == 'old') echo 'class="active"'?> href="<?php echo $question['Question']['moo_href']?>?tab=old&app_no_tab=1"><?php echo __d('question','Oldest');?></a></li>
					</ul>
	        	</div>
	        </div>
	        <?php if (count($answers)):?>
	        	...
	        	<div class="row question-main-content item_answer" id="answer_<?php echo $answer['QuestionAnswer']['id']?>">
	        		<?php $comments = $commentModel->getComments("Answer",$answer['QuestionAnswer']['id']);?>
					<div class="col-md-1 col-xs-1 vote-block">
					   <!-- vote group -->
					   <ul>
					...
					<div class="list_question col-md-11 col-xs-10">
					            ...
					            <div class="list_option">
					                <div class="dropdown">
					                    ...
					                    <ul class="dropdown-menu" role="menu" aria-labelledby="dLabel">
					                        <li><a href="<?php echo $this->base;?>/questions/edit_answer/<?php echo $answer['QuestionAnswer']['id'];?>" title="<?php echo __d('question','Edit Answer')?>"><?php echo __d('question','Edit Answer')?></a></li>
						...
						<div class="row question_info">
							<div class="clear">
								<a href="<?php echo $answer['User']['moo_href']?>">
									<span class="author-avatar">
						...
						<div class="comments-container">
							<?php echo $this->element('comments',array('type'=>'Answer','id'=>$answer['QuestionAnswer']['id'],'comments'=>$comments,'comment_count'=>$answer['QuestionAnswer']['comment_count']),array('plugin'=>'Question'));?>
						</div>
					</div>
	        	</div>
	        	<?php endforeach;?>
	        ...
	        <?php if ($helper->canAnswer($question,$uid)):?>
		        <div class="row form-reply">
		        	<div class="col-md-12">
		        		...
		        		<form id="form_reply" action="<?php echo $this->base?>/questions/post_answer?app_no_tab=1" method="post" class="question_form_reply create_form">
		        			...
		        			<div class="row submit-wrapper">
		        				<div class="col-md-2">
	                                <button id="submit_reply" class="mdl-button mdl-js-button mdl-js-ripple-effect mdl-button--raised mdl-button--colored1"><?php echo __d('question','Post answer');?></button>
	                            </div>
		        			</div>
		        		</form>
		        	</div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button id="edit_answer_button" type="button" class="mdl-button mdl-js-button mdl-js-ripple-effect mdl-button--raised mdl-button--colored1"><?php echo __d('question','Save changes');?></button>
      </div>
    </div><!-- /.modal-content -->
  </div><!-- /.modal-dialog -->
</div><!-- /.modal -->