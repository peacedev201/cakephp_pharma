    ...
    foreach ($groups as $group):
        ...
        <li class="full_content p_m_10" <?php if ($i == count($groups)) echo 'style="border-bottom:0"'; ?>>
            <a href="<?php echo  $this->request->base ?>/groups/view/<?php echo  $group['Group']['id'] ?>/<?php echo  seoUrl($group['Group']['name']) ?>">
                <img src="<?php echo $groupHelper->getImage($group, array('prefix' => '300_square'))?>" class="group-thumb" />
            </a>
            <div class="group-info">
                <a class="title" href="<?php echo  $this->request->base ?>/groups/view/<?php echo  $group['Group']['id'] ?>/<?php echo  seoUrl($group['Group']['name']) ?>">
                    <b><?php echo  h($group['Group']['name']) ?></b>
                </a>
            ...
            <?php if (!empty($uid) && ( ( !empty($group['Group']['my_status']) && $group['Group']['my_status']['GroupUser']['status'] == GROUP_USER_ADMIN ) || !empty($cuser['Role']['is_admin'] )) ): ?>
                <div class="list_option">
                    ...
                    <div class="dropdown">
                        <button class="btn btn-default" id="dropdown-edit" data-target="#" data-toggle="dropdown">
