...
<?php foreach ($categories as $key => $cat): ?>
    <?php if($key == 1): ?>
    <li>
    <span id="cat_more" class="visible-xs visible-sm"><i class="material-icons">add</i></span>
    <ul class="cat_more dropdown-menu" >
    <?php endif; ?>
    <?php if ($cat['Category']['header']): ?>
        <li class="category_header">
             <em id="cat_menu_<?php echo $cat_id; ?>"  ><?php echo $cat['Category']['name'] ?></em>
                <ul class="sub_cat dropdown-menu" ">
