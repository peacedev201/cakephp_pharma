...
if (!empty($albums) && count($albums) > 0) {
    foreach ($albums as $album):
    	...
    	{    		
	    	...
	    	if ($photo)
	    	{
	    		$covert = $photoHelper->getImage($photo, array('prefix' => '300_square'));
	    	}
	    	else
	    	{
	    		$covert = $photoHelper->getAlbumCover('', array('prefix' => '300_square'));
	    	}
    	}
    	else
    	{
    		$covert = $photoHelper->getAlbumCover($album['Album']['cover'], array('prefix' => '300_square'));
    	}
        ?>
