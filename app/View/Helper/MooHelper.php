...
class MooHelper extends AppHelper implements CakeEventListener{
    ...
    public function getItemPhoto($obj, $options_link = array(), $options_image = array(),$linkOnly=false) {
        ...
        */
        $url_hook = new CakeEvent('Helper.getItemPhoto.url_image', array('obj'=>$obj,'size' => intval($prefix)));
        $this->getEventManager()->dispatch($url_hook);
        (!empty($url_hook->result))? $thumbUrl = $url_hook->result : $thumbUrl;
    ...
    public function getImageUrl($obj, $options = array(),$forceLocal=false) {
        ...
        */
        $url_hook = new CakeEvent('Helper.getItemPhoto.url_image', array('obj'=>$obj,'size' => intval($prefix)));
        $this->getEventManager()->dispatch($url_hook);
        (!empty($url_hook->result))? $url = $url_hook->result : $url;
        return $url;
    }
