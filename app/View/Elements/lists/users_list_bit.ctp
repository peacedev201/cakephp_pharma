...
?>
        <?php
        $univModel = MooCore::getInstance()->getModel('University');
        $univ = $univModel->findByCode($user['User']['university_id']);
        ?>
 ...
 ?>
		...
		<div class="user-list-info">
			...
			<div class="">
                <?php if($user['User']['specialty'] == SPECIALTY_STUDENT):?>
                    <?php if(!empty($univ) && $user['User']['show_info_univ']):?>
                        <div class="date"> <?php echo $univ['University']['name']; ?></div>
                    <?php endif;?>
                    <?php if($user['User']['show_info_title']):?>
                        <div class="date"> <?php echo __('Student'); ?></div>
                    <?php endif;?>
                <?php else:?>
                    <?php if(!empty($user['User']['com_name']) && $user['User']['show_info_company']):?>
                        <div class="date"> <?php echo $user['User']['com_name']; ?></div>
                    <?php endif; ?>
                    <?php if(!empty($user['User']['com_title']) && $user['User']['show_info_title']):?>
                        <div class="date"> <?php echo $user['User']['com_title']; ?>
                        <?php
                        if($user['User']['specialty'] == SPECIALTY_PHARMACIST && in_array($user['User']['job_belong_to'], array('pharmacy','hosp_pharmacy'))){
                            if(!empty($univ) && $user['User']['show_info_univ']){
                                echo '('.$univ['University']['name'].')';
                            }
                        }else if($user['User']['com_department'] && $user['User']['com_department'] != 'others'){
                            echo '('.$user['User']['com_department'].')';
                        }
                        ?>
                        </div>
                    <?php endif;?>
