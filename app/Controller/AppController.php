...
class AppController extends Controller
{
    ...
    */
    ...
    private function doThemeProcess()
    {
        ...
        //Register process
        if(!$this->request->is('ajax')) {
            $pass_action = array('home_ajax_lang', 'home_ajax_theme', 'home_do_language', 'home_do_theme', 'users_step_2', 'users_step_3', 'users_save_step_2', 'users_save_step_3', 'users_find_by_phone', 'users_search_sale_area');
            $url = ($this->params['plugin'] ? $this->params['plugin'] . '_' : '');
            $url .= $this->params['controller'] . '_' . $this->params['action'];
            $cuser = $this->_getUser();
            if (isset($cuser['Role']) && $cuser['Role']['is_super'] != 1 &&
                $this->params['prefix'] != 'admin' && !in_array($url, $pass_action) &&
                $this->params['plugin'] != 'subscription' &&
                ($this->params['controller'] != 'users' ||
                    ($this->params['controller'] == 'users' && $this->params['action'] != 'do_logout'))
            ) {
                if ($cuser['step'] == 0) {
                    $this->Session->setFlash(__('Please finish all steps to start using the site.'),
                        'default', array('class' => 'Metronic-alerts alert alert-danger fade in'), 'confirm_remind');
                    $this->redirect('/users/step_2');
                }
                if ($cuser['step'] == 2) {
                    $this->redirect('/users/step_3');
                }
            }
        }
    ...
    private function doViewerProcess()
    {
        ...
        if (!empty($cuser) && !$cuser['confirmed'] && Configure::read('core.email_validation')) {
            if($this->request->action == 'profile_1'){
                $this->Session->setFlash(__('Your new email address has not been confirmed!! It would cause many inconvenience in using key functions such Friend\'s contact.! We recommend members to confirm your email and other contact informations'),
