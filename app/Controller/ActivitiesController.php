...
class ActivitiesController extends AppController {
    ...
    public function ajax_share() {
        ...
        if(!empty($this->request->data['privacy'])){
            $cuser = $this->_getUser();
            switch ($this->request->data['privacy']){
                case MY_REGION:
                    if($cuser['job_belong_to'] == 'pharmacy'){
                        $this->request->data['region'] = $cuser['kpasame'];
                    }elseif($cuser['com_department'] == 'sales' && !in_array($cuser['job_belong_to'], array('pharmacy_chain', 'it_software'))){
                        if(empty($cuser['sale_area'])){
                            echo 'miss_sale_area';
                            exit();
                        }
                        $this->request->data['region'] = $cuser['sale_area'];
                    }else{
                        $this->request->data['region'] = $cuser['sidocode'];
                    }
                    break;
                case MY_JOB_AREA:
                    $this->request->data['job_area'] = $cuser['job_belong_to'];
                    break;
            }
        }
