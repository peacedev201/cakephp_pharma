...
class UsersController extends AppController {
    ...
    public function index($type = 'home') {
            ...
            case 'search':
                ...
                foreach ($this->request->data as $field => $value) {
                    if (strpos($field, 'field_') === 0 && !empty($value)) {
                        ...
                        $field_id = $field_id[1];
                        $profile_params = array();
                        $profile_params['ProfileFieldValue.profile_field_id'] = $field_id;
                        if (is_array($value)) {

                            foreach ($value as $item){
                                $profile_params['OR'][]['MATCH(ProfileFieldValue.value) AGAINST("\"?\"" IN BOOLEAN MODE)'] = urldecode($item);
                            }

                        } else
                            $profile_params['ProfileFieldValue.value'] = $value;

                        $tmp['OR'][] = $profile_params;
                        $count++;
                    }
                }
                $profile_params = $tmp;
                if (!empty($profile_params)) {
                    ...
                    $user_ids = $this->ProfileFieldValue->find('list', array('conditions' => $profile_params,
                        ...
                        'having' => array('COUNT(ProfileFieldValue.user_id) >' => $count)
                    ));
        ...
        $this->set(compact('profile_type','profile_type_id','custom_fields', 'values', 'users', 'more_result','loggedInUser'));
 ...
 */
    public function ajax_browse($type = null, $param = null,$isRedirect=true) {
        ...
        switch ($type) {
            ...
            case 'pharmacists':
                $conditions = [
                    'User.specialty' =>  1,
                    'User.kpasame'   =>  $this->Auth->user('kpasame')
                ];
                $users = $this->User->getUsers($page, $conditions, USERS_BROWSE_LIMIT);
                $more_users = $this->User->getUsers($page + 1, $conditions, USERS_BROWSE_LIMIT);
                if (!empty($more_users))
                    $more_result = 1;
                break;
            case 'com_department':
                $conditions = [
                    'User.com_department' =>  'sales',
                    'User.sale_area LIKE'   =>  '%'.$this->Auth->user('sale_area').'%'
                ];
                $users = $this->User->getUsers($page, $conditions, USERS_BROWSE_LIMIT);
                $more_users = $this->User->getUsers($page + 1, $conditions, USERS_BROWSE_LIMIT);
                if (!empty($more_users))
                    $more_result = 1;
                break;
            case 'pharmacy_com_department':
                $conditions = [
                    'User.com_department' =>  'sales',
                    'User.sale_area LIKE'   =>  '%'.$this->Auth->user('kpasame').'%'
                ];
                $users = $this->User->getUsers($page, $conditions, USERS_BROWSE_LIMIT);
                $more_users = $this->User->getUsers($page + 1, $conditions, USERS_BROWSE_LIMIT);
                if (!empty($more_users))
    ...
    public function ajax_signup_step1()
    {
        ...
        if ( Configure::read('core.enable_registration_code') && $this->request->data['registration_code'] != Configure::read('core.registration_code') )
        {
            $response = array(
                'result' => 0,
                'error' => __('Invalid registration code')
            );
        ...
        if ( $this->isBanned($this->request->data['email']))
        {
            $response = array(
                'result' => 0,
                'error' => __('You are not allowed to register with this email')
            );
            echo json_encode($response);exit;
        }
        ...
        if ( Configure::read('core.enable_spam_challenge') )
        {
            ...
            if (!empty($challenge) && $challenge['SpamChallenge']['active']){
                ...
                if ( !$found )
                {
                    $response = array(
                        'result' => 0,
                        'error' => __('Invalid security question')
                    );
                    echo json_encode($response);exit;
                }
            }
        ...
        if ($checkRecaptcha)
        {
            ...
            if ($resp != null && !$resp->success) {
                $response = array(
                    'result' => 0,
                    'error' => __('Invalid security code')
                );
                echo json_encode($response);exit;
            }
        ...
        {
            $this->loadModel('Pharmacist');
            $item = $this->Pharmacist->getSameArea($data['email']);
            if(!empty($item)){
                $data['kpasame'] = $item['Pharmacist']['kpasame'];
                $data['sidocode'] = $item['Pharmacist']['sidocode'] ;
            }

            $this->User->set( $data );
            $this->User->save();
            $this->getEventManager()->dispatch(new CakeEvent('UserController.doSaveUser', $this, array('data' => $data)));
            $this->getEventManager()->dispatch(new CakeEvent('UserController.doAfterRegister', $this, array('data' => $data)));
            // Log user in
            $user = $this->User->read();
            $cuser = $user['User'];
            ...
            $response = array(
                'result' => 1,
                'redirect' => $this->request->base.'/users/step_2'
            );
...
//            $this->set('profile_type',$profile_type);
    ...
    private function _editProfile( $uid = null , $adminEdit = false, $ajax = false)
    {
        ...
        if (!empty($this->request->data)) {
            ...
            $univ_style = '';
            switch ($user_old['User']['specialty']){
                case SPECIALTY_OTHER:
                case 0:
                    $univ_style = 'display:none';
                    break;
            ...
            $uniField = 'name';
            if(Configure::read('Config.language') == 'kor')
                $uniField = 'name_kor';

            $this->set(compact('universities','univ_style','uniField'));
    ...
    public function password($isRedirect=true)
    {
        ...
        if (!empty($this->request->data))
        {
            ...
            $array_remove_validates = array('birthday','gender','username','timezone','about','com_phone','uni_grade','com_department','sale_area', 'represent_email');
    ...
    public function resetpass( $code = null )
    {
        ...
        if ( !empty( $this->request->data ) )
        {
            ...
            $this->User->validator()->remove('specialty');
            $this->User->validator()->remove('job_belong_to');
            $this->User->validator()->remove('university_id');
            $this->User->validator()->remove('admission_year');
            $this->User->validator()->remove('mobile');
            $this->User->validator()->remove('sub_mail');
            $this->User->validator()->remove('com_phone');
            $this->User->validator()->remove('com_name');
            $this->User->validator()->remove('com_zip');
            $this->User->validator()->remove('uni_grade');
            $this->User->validator()->remove('com_department');
            $this->User->validator()->remove('sale_area');
            $this->User->validator()->remove('represent_email');
    ...
    public function member_login(){
        ...
        if(isset($this->request->named['social'])){
    ...
    public function resend_validation($skip_check = 1){
        ...
        if(empty($cuser['User']['code'])){
            $code = md5( Configure::read('Security.salt') . time() );
        }else{
            $code = $cuser['User']['code'];
        }

        if(!empty($this->request->data['email'])){
            $email = $this->request->data['email'];
            if(!$skip_check && $email == $cuser['User']['email']){
                $skip_check = 0;
            }else{
                $skip_check = 1;
            }
        ...
        if(!empty($email) && $skip_check) {
            ...
            if(!empty($this->request->data['email'])) {
                ...
                if($cuser['User']['mail_to'] == $cuser['User']['email']){
                    $data_save['mail_to'] = $this->request->data['email'];
                }

                $this->User->id = $cuser['User']['id'];
                $this->User->set($data_save);
                if ($this->User->validates()) {
                    $this->User->save();

                    $this->loadModel('EmailRevert');
                    $this->EmailRevert->deleteAllItem($cuser['User']['id']);
                    $this->EmailRevert->save(array(
                        'user_id' => $cuser['User']['id'],
                        'email' => $cuser['User']['email'],
                        'confirm' => $cuser['User']['confirmed'],
                    ));
            ...
            $this->MooMail->send($email, 'welcome_user_confirm',
                array(
                    'email' => $email,
                    'recipient_title' => $cuser['User']['name'],
                    'recipient_link' => $http . '://' . $_SERVER['SERVER_NAME'] . $cuser['User']['moo_href'],
                    'site_name' => Configure::read('core.site_name'),
                    'confirm_link' => $http . '://' . $_SERVER['SERVER_NAME'] . $this->request->base . '/users/do_confirm/' . $code,
                )
            );
        }
        ...
        if(!$skip_check){
            echo json_encode(array(
                'status' => 0,
                'message' => __('This your current email')
            ));
        }else {
            echo json_encode(array(
                'status' => 1
    ...
    // resend validation user sub email
    public function resend_sub_validation($skip_check = 1){
        ...
        if(empty($cuser['User']['submail_confirm_code'])){
            $code = md5( Configure::read('Security.salt') . time() );
        }else{
            $code = $cuser['User']['submail_confirm_code'];
        }

        if(!empty($this->request->data['email'])){
            $email = $this->request->data['email'];
            if(!$skip_check && $email == $cuser['User']['sub_mail']){
                $skip_check = 0;
            }else{
                $skip_check = 1;
            }
        }else{
            $email = $cuser['User']['sub_mail'];
        }
        ...
        if(!empty($email) && $skip_check) {
            ...
            if(!empty($this->request->data['email'])) {
                ...
                $data_save = array(
                    'sub_mail' => $this->request->data['email'],
                    'submail_confirmed' => 0,
                    'submail_confirm_code' => $code,
                );

                if($cuser['User']['mail_to'] == $cuser['User']['sub_mail']){
                    $data_save['mail_to'] = $this->request->data['email'];
                }
                ...
                if ($this->User->validates()) {
                    ...
                    $this->loadModel('EmailRevert');
                    $this->EmailRevert->deleteAllItem($cuser['User']['id'], 1);
                    $this->EmailRevert->save(array(
                        'user_id' => $cuser['User']['id'],
                        'email' => $cuser['User']['sub_mail'],
                        'confirm' => $cuser['User']['submail_confirmed'],
                        'sub' => 1,
                    ));
                } else {
                    $errors = $this->User->invalidFields();
            ...
            $this->MooMail->send($email, 'confirm_sub_email', array(
                    'email' => $email,
                    'recipient_title' => $cuser['User']['name'],
                    'recipient_link' => $http . '://' . $_SERVER['SERVER_NAME'] . $cuser['User']['moo_href'],
                    'site_name' => Configure::read('core.site_name'),
                    'confirm_link' => $http . '://' . $_SERVER['SERVER_NAME'] . $this->request->base . '/users/submail_confirm/' . $code
                )
            );
        }
    ...
    public function step_2(){
        ...
        $this->loadModel('ProfileFieldValue');
        $values  = array();
        $vals = $this->ProfileFieldValue->getValues($cuser['id']);
        // format the profile field values array
        foreach ($vals as $val) {
            $values[$val['ProfileFieldValue']['profile_field_id']] = array('id' => $val['ProfileFieldValue']['id'],
                'value' => $val['ProfileFieldValue']['value']);
    ...
    public function save_step_2(){
        ...
        if(!empty($this->request->data)){
            ...
            $data['confirmed']  = ( Configure::read('core.email_validation') && !$cuser['is_social'] ) ? 0 : 1;

            switch ($data['specialty']){
                case SPECIALTY_STUDENT:
                    unset($this->User->validate['job_belong_to']);
                    break;
                case SPECIALTY_OTHER:
                    unset($this->User->validate['university_id']);
                    unset($this->User->validate['admission_year']);
                    break;
            }
            if(!empty($data['university'])){
                ...
                $this->loadModel('University');
                $univ = $this->University->findByCode($data['university_id']);
                $data['university'] = !empty($univ) ? $univ['University']['name'] : '';
            }
            ...
            {
                $errors = $this->User->invalidFields();
                $response = array('result' => 0, 'message'=> current( current( $errors ) ));
                echo json_encode($response);exit;
            }
            $require_upload_avatar = Configure::read('core.require_upload_avatar');
            if(!empty($require_upload_avatar) && empty($data['avatar']))
            {
                $response = array('result' => 0, 'message'=> __('Avatar is required'));
                echo json_encode($response);exit;
            }
            ...
            foreach ($custom_fields as $field)
            {
                ...
                if ( $field['ProfileField']['required'] && empty( $value ) && !is_numeric( $value ) )
                {
                    $response = array('result' => 0, 'message'=>  $field['ProfileField']['name'] . __(' is required'));
                    echo json_encode($response);exit;
                }
            }
            ...
            if ( $this->User->validates() )
            {
                ...
                //confirm sub email
                if($cuser['sub_mail'] != $data['sub_mail']) {
                    $code = md5( Configure::read('Security.salt') . time() );
                    $http = (!empty($ssl_mode)) ? 'https' : 'http';

                    $this->MooMail->send($data['sub_mail'], 'confirm_sub_email', array(
                            'email' => $data['sub_mail'],
                            'recipient_title' => $cuser['name'],
                    ...
                    $data['submail_confirm_code'] = $code;
                }

                if($cuser['avatar'] != $data['avatar']) {
                    // keep a copy of avatar for Profile Album picture, because after uploaded, behavior deleted original file
                    $newTmpAvatar = '';
                    if (!empty($data['avatar'])) {
                        $file = $data['avatar'];
                ...
                if($this->User->save()){
                    ...
                    if(!empty($newTmpAvatar))
                    {
                        ...
                        $this->Photo->save();
                        $this->Album->id = $album_id;
                        $filename = explode(DS, $tmp_photo_url);
                        $filename1 = $filename[count($filename) - 1];
                        $this->Album->save(array('cover' => $filename1));
                    ...
                    $user = $this->User->read();
                    $user['User']['specialty'] = $data['specialty'];
                    if($this->Session->read('replace_avatar') == '0'){
                        $replace = 0;
                    }else{
                        $replace = (!empty($newTmpAvatar) || $user['User']['is_social']) ? 0 : 1;
                    }
                    $this->Session->write('replace_avatar', $replace);
                    $this->getEventManager()->dispatch(new CakeEvent('UserController.doAfterStep2', $this, array('User' => $user['User'], 'replace' => $replace)));
                }
    ...
    private function _step_3(){
        $cuser = $this->_getUser();
        $grade = array(
            'freshman' => __('Freshman'),
            'sophomore' => __('Sophomore'),
            'junior' => __('Junior'),
            'senior' => __('Senior'),
        );
        ...
        $sale_areas = array();
        if(!empty($cuser['sale_area'])){
            $this->loadModel('Sigungu');
            $items = $this->Sigungu->find('all', array(
                'conditions' => array('Sigungu.kpasame' => explode(',',$cuser['sale_area'])),
                'fields' => array('Sigungu.id', $field),
                'group' => 'Sigungu.kpasame'
            ));
            foreach ($items as $item){
                $sale_areas[] = array('id' => $item['Sigungu']['id'], 'name' => $item['Sigungu'][$field]);
            }
        }

        $jobs = array(
            'pharmacy' => array('name' => __('Pharmacy'), 'class' =>'hide-sale '),
            'hosp_pharmacy' => array('name' => __('Hospital Pharmacy'), 'class' =>'hide-sale '),
            'pharma_company' => array('name' => __('Pharma-company'), 'class' =>''),
            'wholesaler' => array('name' => __('Wholesaler'), 'class' =>''),
            'supplier' => array('name' => __('General supplier'), 'class' =>''),
            'cso' => array('name' => __('CSO'), 'class' =>''),
            'consumables' => array('name' => __('Consumables'), 'class' =>''),
            'pharmacy_chain' => array('name' => __('Pharmacy chain'), 'class' =>''),
            'it_software' => array('name' => __('It/Software'), 'class' =>''),
            'government' => array('name' => __('Government'), 'class' =>'hide-sale '),
            'newspaper' => array('name' => __('Newspaper'), 'class' =>'hide-sale '),
            'other_company' => array('name' => __('Other Company'), 'class' =>''),
            'others' => array('name' => __('Others'), 'class' =>''),
        );

        $interest_jobs = array(
            'pharmacy' => __('Pharmacy'),
            'hosp_pharmacy' => __('Hospital Pharmacy'),
            'pharma_company' => __('Pharma-company'),
            'wholesaler' => __('Wholesaler'),
            'supplier' =>  __('General supplier'),
            'cso' => __('CSO'),
            'consumables' => __('Consumables'),
            'pharmacy_chain' => __('Pharmacy chain'),
            'it_software' => __('It/Software'),
            'government' => __('Government'),
            'newspaper' => __('Newspaper'),
            'other_company' => __('Other Company'),
            'at_home' =>  __('At home'),
        );
        ...
        $places = array(
            ...
            'Ulsan' => __('Ulsan'),
            'Sejong' => __('Sejong'),
            'Gyeonggi' => __('Gyeonggi'),
            'Gangwon' => __('Gangwon'),
            'Chungbuk' => __('Chungbuk'),
            'Chungnam' => __('Chungnam'),
            'Jeonnam' =>  __('Jeonnam'),
            'Jeonbuk' =>  __('Jeonbuk'),
            'Gyeongbuk' =>  __('Gyeongbuk'),
            'Gyeongnam' =>  __('Gyeongnam'),
            'Jeju' =>  __('Jeju'),
        );

        $this->loadModel('Sido');
        $sidos = $this->Sido->getAllSido();

        $this->set(compact('sale_areas','grade', 'jobs', 'interest_jobs', 'places', 'sidos'));
    }
    ...
    public function save_step_3(){
        ...
        if($cuser['specialty'] == SPECIALTY_STUDENT) {
            unset($this->User->validate['com_phone']);
            unset($this->User->validate['com_name']);
            unset($this->User->validate['com_zip']);
            unset($this->User->validate['com_department']);
            unset($this->User->validate['sale_area']);
            ...
            if ($data['job_belong_to'] == 'others') {
                if(isset($this->User->validate['com_phone']))  unset($this->User->validate['com_phone']);
                if(isset($this->User->validate['com_name'])) unset($this->User->validate['com_name']);
                if(isset($this->User->validate['com_zip'])) unset($this->User->validate['com_zip']);
                if(isset($this->User->validate['com_department'])) unset($this->User->validate['com_department']);
                if(isset($this->User->validate['sale_area'])) unset($this->User->validate['sale_area']);
                $data['work_at_home'] = 1;

                if(!empty($data['job_interest_text'])){
                    $data['job_interest'] = $data['job_interest_text'];
                }
                if(!empty($data['sidocode'])){
                    $data['kpasame'] = '';
                }
            }else{
                $data['work_at_home'] = 0;
            }
            if (!$data['require_department']) {
                if(isset($this->User->validate['com_department']))  unset($this->User->validate['com_department']);
                if(isset($this->User->validate['sale_area'])) unset($this->User->validate['sale_area']);
            }
            if ($data['com_department'] != 'sales') {
                if(isset($this->User->validate['sale_area'])) unset($this->User->validate['sale_area']);
            }
            if(in_array($data['job_belong_to'], array('pharmacy_chain', 'it_software'))){
                if(isset($this->User->validate['sale_area'])) unset($this->User->validate['sale_area']);
            }

            $temp_phone = '';
            if (isset($data['com_phone'])) {
                $com_phones = $data['com_phone'];
                $data['com_phone'] = '';
                $i = 0;
                foreach ($com_phones as $com_phone) {
                    $data['com_phone'] .= $com_phone;
                    $temp_phone .= $com_phone;
                    if ($i < 2) {
                        $temp_phone .= '-';
                    }
                    $i++;
            ...
            $data['com_phone'] = $data['temp_phone'];
        }
        ...
        {
            if($cuser['specialty'] == SPECIALTY_STUDENT) {
                if(!empty($data['represent_email'])){
                    $code = md5( Configure::read('Security.salt') . time() );
                    $http = (!empty($ssl_mode)) ? 'https' : 'http';
                    $this->MooMail->send($data['represent_email'],'confirm_student',
                        array(
                            'name' => $cuser['name'],
                            'univ_name' => $data['com_name'],
                            'yes_link' => $http.'://'.$_SERVER['SERVER_NAME']. $this->request->base.'/users/student_confirm/'.$code,
                            'no_link'=> $http.'://'.$_SERVER['SERVER_NAME']. $this->request->base ,
                        )
                    );
                    $data['student_confirm_code'] = $code;
                }
            }else{
                $data['com_phone'] = $temp_phone;
                if(!empty($this->User->validate)) {
                    unset($this->User->validate);
                }

                if(empty( $data['kpasame']) &&  empty($data['sidocode'])){
                    $this->loadModel('PharmaBook');
                    $item = $this->PharmaBook->getByPhone($temp_phone);
                    if(!empty($item)){
                        $data['kpasame'] = $item['PharmaBook']['kpasame'];
                        $data['sidocode'] = $item['PharmaBook']['sidocode'];
                    }else{
                        $this->loadModel('ZipCode');
                        $item = $this->ZipCode->getByZipcode($data['com_zip']);
                        if(!empty($item)){
                            $data['kpasame'] = $item['ZipCode']['kpasame'];
                            $data['sidocode'] = $item['ZipCode']['sidocode'] ;
                        }
                    }
                }
            }
            ...
            $user = $this->User->read();
            if($user['User']['specialty'] == SPECIALTY_STUDENT){
                $profile_group = GROUP_STUDENT;
            }elseif($user['User']['specialty'] == SPECIALTY_PHARMACIST && $user['User']['job_belong_to'] == 'others'){
                $profile_group = PHARMACIST_OTHER;
            }elseif($user['User']['specialty'] == SPECIALTY_PHARMACIST && $user['User']['com_department'] == 'sales' && in_array($user['User']['job_belong_to'], array('pharma_company','wholesaler','supplier','cso','consumables'))){
                $profile_group = PHARMACIST_SALE;
            }elseif($user['User']['specialty'] == SPECIALTY_PHARMACIST){
                $profile_group = PHARMACIST_GENERAL;
            }elseif($user['User']['specialty'] == SPECIALTY_OTHER && $user['User']['job_belong_to'] == 'others'){
                $profile_group = OTHER_OTHER;
            }elseif($user['User']['specialty'] == SPECIALTY_OTHER && $user['User']['com_department'] == 'sales' && in_array($user['User']['job_belong_to'], array('pharma_company','wholesaler','supplier','cso','consumables'))){
                $profile_group = OTHER_SALE;
            }elseif($user['User']['specialty'] == SPECIALTY_OTHER){
                $profile_group = OTHER_GENERAL;
            }
            if(!empty($this->User->validate)){
                unset($this->User->validate);
            }
            $this->User->clear();
            $this->User->id = $user['User']['id'];
            $this->User->saveField('profile_group', $profile_group);


            if ($cuser['specialty'] == SPECIALTY_OTHER) {
                if($data['com_department'] == 'sales' ){
                    $cuser['specialty'] = 4;//cheat for sale avatar
                }
                $replace = $this->Session->read('replace_avatar');
                $this->getEventManager()->dispatch(new CakeEvent('UserController.doAfterStep2', $this, array('User' => $cuser, 'replace' => $replace)));
            }
    ...
    public function find_by_phone(){
        $this->autoRender = false;
        $response = array('result' => 0);
        if(!empty($this->request->data['phone'])){
            $this->loadModel('PharmaBook');
            $result = $this->PharmaBook->getByPhone($this->request->data['phone']);
            if(!empty($result)){
                $response = array(
                    'result' => 1,
                    'name' => $result['PharmaBook']['company'],
                    'zip' => $result['PharmaBook']['zip_code'],
                    'addr_1' => $result['PharmaBook']['address_1'],
                    'addr_2' => $result['PharmaBook']['address_2'],
                    'fax' => $result['PharmaBook']['fax'],
                );
            }
        }
        echo json_encode($response);
    ...
    public function search_sale_area(){
        $this->autoRender = false;
        $this->loadModel('Sigungu');
        ...
        $items = $this->Sigungu->getSigungus(array('Sigungu.'.$field.' LIKE "%'. $this->request->query['q'].'%"'));
        $results = array();
        foreach ($items as $item){
            $results[] = array(
                'id' => $item['Sigungu']['kpasame'],
                'name' => $item['Sigungu'][$field],
            );
        }
        echo json_encode($results);
    }

    public function student_confirm($code = null){
        $msg = __('Confirm code is incorrect');
        if(!empty($code)) {
            $user = $this->User->find('first', array(
                'conditions' => array(
                    'User.student_confirm_code' => $code
                )
            ));
            if(!empty($user)){
                $this->User->id = $user['User']['id'];
                $this->User->save(array('student_confirm' => 1));
                $msg = __('Confirm successfully');
            }
        }
        $this->set('msg', $msg);
    }
    ...
    public function step_4(){
        ...
        $this->_profileCompleteness();
    }

    public function zip_suggestion(){
        $this->autoRender = false;
        $searchVal = isset($this->request->data['searchVal']) ? $this->request->data['searchVal'] : '';
        if($searchVal){
            $this->loadModel('ZipCode');
            $param = array('zip_code LIKE "' . $searchVal.'%"');
            $zipcodes = $this->ZipCode->getZipcodes($param);

            $html = '';
            foreach ($zipcodes as $code){
                $html .= '<li>'.$code['ZipCode']['zip_code'].'</li>';
            }
            echo $html;
        }
    }

    public function submail_confirm( $code = null )
    {
        $this->autoRender = false;
        $user = $this->User->find('first', array(
            'conditions' => array('User.submail_confirm_code' => $code)
        ));

        if ( !empty(  $user ) )
        {
            $this->User->id = $user['User']['id'];
            $this->User->save( array( 'submail_confirmed' => 1 ) );
            $this->Session->delete('Message.confirm_remind');
            $this->Session->setFlash( __('Your sub email has been validated!') );
        }
        else
            $this->Session->setFlash( __('Invalid code!'), 'default', array('class' => 'error-message') );
    ...
    private function _profileCompleteness(){
        ...
        if(!empty($viewer) && Configure::read('ProfileCompletion.profile_completion_enabled')){
            ...
            $profile_completion = $this->ProfileCompletion->find('list', array('fields' => array('field_name', 'field_value'), 'conditions' => array('ProfileCompletion.profile_type_id' => $profile_type_id, 'ProfileCompletion.field_value <>' => 0)));
            ...
            if(!empty($profile_fields)){
                foreach ($profile_fields as $k => $val) {
                    if($val['ProfileField']['type'] == 'location'){
                        ...
                        if((empty($tmp_profile_fields_val) || $tmp_profile_fields_val['ProfileFieldValue']['value'] == '') && !empty($tmp_profile_completion['fields_'.$val['ProfileField']['id']])){
                            $tmp_profile_completion['fields_'.$val['ProfileField']['id']] = $profile_completion['fields_'.$val['ProfileField']['id']];
                            unset($profile_completion['fields_'.$val['ProfileField']['id']]);
                        }
                    }
    ...
    public function save_setting(){
        $this->autoRender = false;
        $uid = $this->Auth->user('id');
        if($this->request->is('post') && $uid){
            $this->User->id = $uid;
            $data = $this->request->data;
            $this->User->save(array(
                'show_info_company' => $data['show_info_company'],
                'show_info_univ' => $data['show_info_univ'],
                'show_info_title' => $data['show_info_title'],
                'show_avatar' => $data['show_avatar'],
            ));
        }
    }

    public function get_zipcode_api(){
        $this->autoRender = false;
        $result = array('sidocode' => '', 'kpasame' => '');
        if($this->request->is('post')){
            $data = $this->request->data;
            if(!empty($data['zip_code'])) {
                $this->loadModel('ZipCode');
                $item = $this->ZipCode->getByZipcode($data['zip_code']);
                if (!empty($item)) {
                    $result['kpasame'] = $item['ZipCode']['kpasame'];
                    $result['sidocode'] = $item['ZipCode']['sidocode'];
                }
            }
        }
        echo json_encode($result);
    ...
    public function ajax_revert($sub = 0){
        $this->autoRender = false;
        $cuser = MooCore::getInstance()->getViewer();
        $uid = $cuser['User']['id'];
        $this->loadModel('EmailRevert');
        $item = $this->EmailRevert->getItem($uid, $sub);

        if(!empty($item)){
            $this->User->id = $uid;
            if($sub){
                $data_save = array(
                    'sub_mail' => $item['EmailRevert']['email'],
                    'submail_confirmed' => $item['EmailRevert']['confirm'],
                );
                if($cuser['User']['mail_to'] == $cuser['User']['sub_mail']){
                    $data_save['mail_to'] = $item['EmailRevert']['email'];
                }
                $this->User->save($data_save);
            }else{
                $data_save = array(
                    'email' => $item['EmailRevert']['email'],
                    'confirmed' => $item['EmailRevert']['confirm'],
                );
                if($cuser['User']['mail_to'] == $cuser['User']['email']){
                    $data_save['mail_to'] = $item['EmailRevert']['email'];
                }
            ...
            $this->EmailRevert->delete($item['EmailRevert']['id']);
        }
    ...
    public function profile_1()
    {
        ...
        $this->loadModel('EmailRevert');
        $revert = $this->EmailRevert->checkExist($uid, 0);
        $sub_revert = $this->EmailRevert->checkExist($uid, 1);
        $this->set('revert', $revert);
        $this->set('sub_revert', $sub_revert);
    }

    public function ajax_save_profile_1($isRedirect=true)
    {
        $this->_checkPermission();
        $uid = $this->Auth->user('id');
        $this->User->id = $uid;
        ...
        if($isRedirect) {
            ...
            $cuser = $this->_getUser();
            if(!$cuser['confirmed']){
                $result['message'] = __('Please kindly confirm your new email address.<br/> %s <br/>If validation email not arrived, use "resend validation email" button.', $cuser['email']);
            }else if( !$cuser['submail_confirmed']){
                $result['message'] = __('Please kindly confirm your new email address.<br/> %s <br/>If validation email not arrived, use "resend validation email" button.', $cuser['sub_mail']);
            }
            $result['status'] = true;
            echo json_encode($result);
            exit;
    ...
    public function ajax_save_profile_2($isRedirect=true)
    {
        $this->_checkPermission();
        $uid = $this->Auth->user('id');
        $this->loadModel("ProfileField");
        ...
        $array_remove_validates = array('email','mobile','com_phone','uni_grade','com_department','sale_area', 'represent_email');
        foreach ($array_remove_validates as $validate)
        {
            if (isset($this->User->validate[$validate]))
                unset($this->User->validate[$validate]);
        ...
        if(!empty($this->request->data['specialty']) && $this->request->data['specialty'] == SPECIALTY_STUDENT){
            $this->loadModel('University');
            $university = $this->University->findByCode($this->request->data['university_id']);
            $this->request->data['kpasame'] = '';

            if(!empty($university)) {
                $this->request->data['sidocode'] = $university['University']['sidocode'];
            }
        }
    ...
    public function ajax_confirm_mail($type = 'mail'){
        $this->set('type', $type);
    }

    public function ajax_update_mail(){
        $this->autoRender = false;
        $response = array(
            'status' => 0,
            'message' => ''
       ...
       if(!empty($this->request->data)){
           $cuser = $this->_getUser();
           if($this->request->data['type'] == 'mail'){
                if($cuser['confirmed']){
                    $response['status'] = 1;
                    $this->User->id = $cuser['id'];
                    ...
                    $response['message'] = __('Please click the validation link to confirm your new email.');
                }
           }else{
               if($cuser['submail_confirmed']){
                   $response['status'] = 1;
                   $this->User->id = $cuser['id'];
                   $this->User->saveField('sub_mail', $this->request->data['email']);
               }else{
                   $response['message'] = __('Please click the validation link to confirm your new email.');
               }
           }
    ...
    public function ajax_sms_verify(){
        if(!empty($this->request->data['sms_verify_phone'])) {
            $phone  = $this->request->data['sms_verify_phone'];
                $code = $this->createRandomCode();
                $msg = __('Your code') . ': ' . $code;
                $param = array(
                    'msg' => $msg,
                    'users' => $phone,
                    'test' => 1,
                );
                $url = 'http://sms.pharmatalk.kr/SMS/send?' . http_build_query($param);
                $ch = curl_init($url);
    ...
    public function ajax_update_phone(){
        $response = array(
            'status' => 0,
            'message' => ''
        );
        ...
        if(!empty($this->request->data)){
            $code = $this->request->data['sms_verify_code'];
            if ($code != $this->Session->read("sms_verify_code"))
            {
                $response['message'] = __('Code invalidate');
            }
            else
            {
                ...
                $this->loadModel("User");
                $this->User->id = $uid;
                if($this->User->save(array('sms_verify' => 1, 'mobile'=>$phone,'sms_verify_checked'=> 1))) {
                    $response['status'] = 1;
