...
class VideoApiListener implements CakeEventListener
{
    ...
    function getVideoInfo($id,$v){
        ...
        $objectsExport =  array(
            ...
            'id' => $object["Video"]['id'],
            'url' => $url,
            'title' => h($v->Text->truncate($object['Video']['title'], 140, array('exact' => false))),
            'images' => $imageArray,
    ...
    function exportVideoCreate($e)
    {
        ...
        list($object,$objectsExport)= $this->getVideoInfo($data['Activity']['item_id'],$v) ;
        ...
        if ($data['Activity']['target_id']) {
            ...
            if ($data['Activity']['type'] == 'Group_Group') {
                if ($show_subject) {
                    $title = $data['User']['name'] . ' > ' . h($object[$name]['moo_title']);
                    $titleHtml = $actorHtml . ' > ' . $v->Html->link(h($object[$name]['moo_title']), FULL_BASE_URL . $object[$name]['moo_href']);
                } else {
    ...
    function exportVideoItemDetailShare($e)
    {
        ...
        list($video,$objectsExport)= $this->getVideoInfo($data['Activity']['parent_id'],$v) ;
