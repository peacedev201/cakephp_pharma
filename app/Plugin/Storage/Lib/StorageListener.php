        ...
        if (key($extra) == 'User') {
            ...
            switch ($extra['User']['show_avatar']){
                ...
                case AVATAR_FRIEND:
                    if(!empty($viewer)){
                        if($viewer['Role']['is_admin']){
                            ...
                            $friends = $friendModel->getFriends($viewer['User']['id']);
                            if (in_array($oid, $friends) || $oid == $viewer['User']['id']) {
                                $show_avatar = 1;
                            }
                        }
                    }
                ...
                case AVATAR_MEMBER:
                    ...
                    break;
                case AVATAR_DISABLE:
                    if(!empty($viewer) && $viewer['User']['id'] == $extra['User']['id']){
                        $show_avatar = 1;
                    }
                    break;
                default:
            ...
            if (!$extra['User']) {
                ...
                if(empty($extra['User']['default_avatar'])){
                    $new_data = new CakeEvent('UserController.doCreateAvatar', array('data' =>$extra['User']));
                    $v->getEventManager()->dispatch($new_data);
                    if(!empty($new_data->result)){
                        $user = $new_data->result;
                        $new_thumb = !empty($user) ? $user['User']['default_avatar'] : '';
                    }
                }else{
                    $new_thumb = $extra['User']['default_avatar'];
                }
                if(!empty($new_thumb)) {
                    $field = $extra[key($extra)]['moo_thumb'];
                    $url = FULL_BASE_LOCAL_URL . $v->request->webroot . 'uploads/' . strtolower(Inflector::pluralize(key($extra))) . '/' . $field . '/' . $extra[key($extra)]['id'] . '/' . $prefix . $new_thumb;
                }else {
                    if ($prefix == '50_square_') {
                        $url = FULL_BASE_LOCAL_URL . $v->request->webroot . strtolower(key($extra)) . '/img/noimage/Male-' . strtolower(key($extra)) . '-sm.png';
                    } else {
