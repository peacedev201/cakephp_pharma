...
(function (root, factory) {
    if (typeof define === 'function' && define.amd) {
        ...
        root.mooFriendinviterTabContent = factory();
    }
}(this, function ($) {


var ddtabcontent = function(tabinterfaceid){
...
ddtabcontent.prototype={
	...
	expandsubcontent:function(subcontentid)
	{
		for (var i = 1; i < this.subcontentids.length - 1; i++)
		{
			...
			subcontent.style.display = (subcontent.id == subcontentid)? "" : "none" //"show" or hide sub content based on matching id attr value
		}
	},
	...
	expandrevcontent:function(associatedrevids){
		...
		for (var i=0; i<allrevids.length; i++){ //Loop through rev attributes for all tabs in this tab interface
			...
			document.getElementById(allrevids[i]).style.display=(associatedrevids.indexOf(","+allrevids[i]+",")!=-1)? "" : "none"
		}
	},
					...
					if(this.id != 0 && this.id != total-1)
					{
						...
						//set active for current tab head
				         //  var page_heads = document.getElementById('contactimporter_page_list').getChildren();
                                         var page_heads = document.getElementById('contactimporter_page_list').childNodes;
				           ...
				           {
				             $(tab_head_selected).parent().addClass('selected');
				           }
				           var sub_page = 5;
				           if((total - 2 - parseInt(this.id)) < 5)
				           {
							...
							if(this.id > 5 && total - 2 > 10)
							{
								...
								{
									document.getElementById(parseInt(this.id) - j).parentNode.style.display = '';
								}
								for(var k = parseInt(this.id); k <= end_page; k ++)
								{
									document.getElementById(k).parentNode.style.display = '';
								}
							}
							if(this.id <= 5 && total - 1 > 10)
							{
								...
								for(var j = 1; j <= max; j ++)
								{
									document.getElementById(j).parentNode.style.display = '';
								}
							...
							if(this.id > 1)
							{
								document.getElementById('0').parentNode.style.display = '';
							}
							else
							{
							...
							{
								document.getElementById(total - 1).parentNode.style.display = '';
							}
				          return false
					}
					else
				          ...
				          var page_heads = document.getElementById('contactimporter_page_list').childNodes;
				           ...
				           if((total - 2 - page_id) < 5)
				           {
							...
							if(page_id > 5 && total - 2 > 10)
							{
								...
								{
									document.getElementById(page_id - j).parentNode.style.display = '';
								}
