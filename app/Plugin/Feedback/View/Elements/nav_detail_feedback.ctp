        ...
        <div class="menu box_content feedback-creator-name">
            ...
            <?php if(!empty($aFeedback['User']['name'])):?>
                <a href="<?php echo $this->request->base?>/users/view/<?php echo $aFeedback['User']['id']?>"> <?php echo h($aFeedback['User']['name'])?></a>     
                 ...
                 <?php if ( isset($friends_request) && in_array($aFeedback['User']['id'], $friends_request) && $aFeedback['User']['id'] != $uid): ?>
                        <a href="<?php echo $this->request->base?>/friends/ajax_cancel/<?php echo $aFeedback['User']['id']?>" id="cancelFriend_<?php echo $aFeedback['User']['id']?>" title="<?php __d('feedback','Cancel Request');?>">
                            <?php echo __d('feedback','Cancel Request')?>
                        </a>
                    <?php elseif ( !empty($respond) && in_array($aFeedback['User']['id'], $respond ) && $aFeedback['User']['id'] != $uid): ?>
                     
                        
                    <?php elseif (isset($friends) && in_array($aFeedback['User']['id'], $friends) && $aFeedback['User']['id'] != $uid): ?>

                        
                    <?php elseif(isset($friends) && isset($friends_request) && !in_array($aFeedback['User']['id'], $friends) && !in_array($aFeedback['User']['id'], $friends_request) && $aFeedback['User']['id'] != $uid): ?>
