...
<?php if (!empty($contest)): ?>
    ...
    <?php endif; ?>
    <h2><?php echo __d('contest', 'Entries') ?></h2>
    <div class="contest_entries _content">
        <?php if(!$no_entries): ?>
            <?php if(!$contest['Contest']['auto_approve']): ?>
                <?php if ($helper->canManageEntries($contest, $viewer)): ?>
                    <div class="contest_nav">
                        <a class="active" href="javascript:void(0);" class="allBtn" data-ctype="approved" data-url="<?php echo $this->request->base ?>/contests/browse_entries/approved/<?php echo $contest['Contest']['id']; ?>"><?php echo __d('contest', 'Approved Entries'); ?></a>
                        <a href="javascript:void(0);" class="pendingBtn" data-ctype="pending" data-url="<?php echo $this->request->base ?>/contests/browse_entries/pending/<?php echo $contest['Contest']['id']; ?>"><?php echo __d('contest', 'Pending Entries'); ?></a>
                    </div>
                <?php endif; ?>
            <?php endif; ?>
        <?php endif; ?>
        ...
        <?php if ($helper->canManageEntries($contest, $viewer)): ?>
            <form id="manage_entries" action="<?php echo $this->request->base ?>/contests/manage_entries" method="post">
