    ...
    function exportGroupJoin($e) {
        ...
        switch (count($groups)) {
            case 1:
                ...
                $joinedHtml = sprintf($joined1, $v->Html->link(h($groups[0]['Group']['name']), FULL_BASE_URL . $groups[0]['Group']['moo_href']));
                break;
            case 2:
                $joined = sprintf($joined2, h($groups[0]['Group']['name']), h($groups[1]['Group']['name']));
                $joinedHtml = sprintf($joined2, $v->Html->link(h($groups[0]['Group']['name']), FULL_BASE_URL . $groups[0]['Group']['moo_href']), $v->Html->link(h($groups[1]['Group']['name']), FULL_BASE_URL . $groups[1]['Group']['moo_href']));
                break;
            case 3:
            default :
                $joined = sprintf($joined3, h($groups[0]['Group']['name']), abs(count($groups) - 1) . ' ' . __('others'));
                $joinedHtml = sprintf($joined3, $v->Html->link(h($groups[0]['Group']['name']), FULL_BASE_URL . $groups[0]['Group']['moo_href']), abs(count($groups) - 1) . ' ' . __('others'));
                break;
        }
