...
<div class="bar-content full_content ">
    <div class="content_center">
        ...
        <div class="mo_breadcrumb bookmark-button">
            ...
            <?php if(( !empty($my_status) && $group['Group']['user_id'] == $uid )): ?>
                <a data-target="#themeModal" data-toggle="modal" class="topButton button button-action" href="<?php echo $this->request->base?>/conversations/ajax_group_send/<?php echo $group['Group']['id'] ?>/<?php echo $group['User']['id'] ?>" title="<?php echo __('Send New Message')?>"  style="margin-right: 35px;color: white;background-color: #33cccc;"><?php echo __('Send Message to All')?></a>
            <?php else: ?>
                <a data-target="#themeModal" data-toggle="modal" class="topButton button button-action" href="<?php echo $this->request->base?>/conversations/ajax_send/<?php echo $group['User']['id'] ?>" title="<?php echo __('Send New Message')?>" style="margin-right: 35px;color: white;background-color: #33cccc;">
                    <?php echo __('Send Message')?>
                </a>
            <?php endif; ?>
            <a href="javascript:void(0);" class="button button-action topButton button-mobi-top shareFeedBtn" share-url="<?php echo $this->Html->url(array(
                                    'plugin' => false,
                                    'controller' => 'share',
                                    'action' => 'ajax_share',
                                    'Group_Group',
            ...
            <?php if ((empty($uid) && !empty($invited_user)) ||
                ...
                <a href="<?php echo  $this->request->base ?>/groups/do_request/<?php echo  $group['Group']['id'] ?>" class="button button-action topButton button-mobi-top join-btn" style="margin-right: 0px;background-color:#f5a624;color: white;"><?php echo  __('Join') ?></a>
            <?php endif; ?>   
            <?php if(!empty($my_status) && $my_status['GroupUser']['status'] == GROUP_USER_ADMIN): ?>
                <a href="<?php echo  $this->request->base ?>/pages/community_level_up/<?php echo  $group['Group']['id'] ?>" class="button button-action topButton button-mobi-top" style="background-color: #ffd966;color: black;"><?php echo __('Level up')?></a>  
            <?php endif; ?>     
            <a href="javascript:void(0);" class="button button-action topButton button-mobi-top" style="background-color: #246ea3;color: white;"><?php echo h($community)?></a>
            <!-- New hook -->
 ...
 ?>
        ...
        <ul class="group-info info">
            ...
            <li>
                <label><?php echo  __('Established') ?>:</label>
                <div>
                    <?= date('Y.m.d',strtotime($group['Group']['created'])) ?>
                </div>
            </li>
            <li><label><?php echo  __('No. of Members') ?>:</label>
                <div><?= $group['Group']['group_active_user_count'] ?></div>
            </li>
            <?php
