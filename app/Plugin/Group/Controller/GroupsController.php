...
class GroupsController extends GroupAppController {
    ...
    public function index($cat_id = null) {
        ...
        // var_dump(APP . 'Plugin/Groups/View' . DS . 'Groups' . DS . $path[0] . '.ctp');die();
    ...
    public function level_up($cat_id = null) {
        //  var_dump(APP . 'Plugin/Groups/View' . DS . 'Groups' . DS . $path[0] . '.ctp');die();
        $cat_id = intval($cat_id);
        ...
        $role_id = $this->_getUserRoleId();
        // caching
        $groups = $this->Group->getGroups(null,$uid,1,null,$role_id,$cat_id);
        $sns_groups = $this->paginate('GroupsDefinition');
  
        $this->set('sns_groups', $sns_groups);
        $this->set('groups', $groups);
        $this->set('cat_id', $cat_id);
        $this->set('title_for_layout', '');
        $this->set('more_result',$more_result);
        ...
        // $group= $this->Group->findById($cat_id);
        // $group['Group']['group_user_count'] = $this->GroupUser->getBlockedUserCount($cat_id);
        // $group['Group']['group_active_user_count'] = $this->GroupUser->getActiveUsers($cat_id);
        ...
        if ($this->request->is('requested')) {
            ...
            // caching
            $group_members = Cache::read('group_' . $cat_id . '_members_widget', 'group');
            if (!$group_members){
                $group_members = $this->GroupUser->getUsers($cat_id, GROUP_USER_MEMBER, null, $num_group_member);
                Cache::write('group_' . $cat_id . '_members_widget', $group_members, 'group');
            }
            
            $group_admins = Cache::read('group_' . $cat_id . '_admins_widget', 'group');
            if (!$group_admins){
                $group_admins = $this->GroupUser->getUsers($cat_id, GROUP_USER_ADMIN, null, $num_group_admin);
                Cache::write('group_' . $cat_id . '_admins_widget', $group_admins, 'group');
            }

            $admin_count = Cache::read('admin_count_group_' . $cat_id, 'group');
            if (!$admin_count){
                $admin_count = $this->GroupUser->getUserCount($cat_id, GROUP_USER_ADMIN);
                Cache::write('admin_count_group_' . $cat_id, $admin_count, 'group');
            }
        ...
        if(empty($user_blocks)){
            $group_admins = Cache::read('group_' . $cat_id . '_admins', 'group');
            if (!$group_admins){ 
                $group_admins = $this->GroupUser->getUsers($cat_id, GROUP_USER_ADMIN, null, 5);
                if (empty($group_admins)) $group_admins = array(0);
                Cache::write('group_' . $cat_id . '_admins', $group_admins, 'group');
            }
            
            $members = Cache::read('group_' . $cat_id . '_members', 'group');
            if (!$members){  
                $members = $this->GroupUser->getUsers($cat_id, GROUP_USER_MEMBER, null, 10);
                if (empty($members)) $members = array(0);
                Cache::write('group_' . $cat_id . '_members', $members, 'group');
            }
        ...
        $photo_count = $this->Photo->getPhotosCount('Group_Group',$cat_id);
        $this->set('photo_count', $photo_count);

        $this->set('group_id', $cat_id);
    ...
    public function update(){
        ...
        $this->Group->saveField("group_status",$this->request->data['value']+1);
        $response['result'] = $this->request->data['id'];
        echo json_encode($response);
    }
    public function update_level(){
        $this->autoRender = false;
        $this->Group->id=$this->request->data['id'];
        $this->Group->saveField("group_status",0);
        $this->Group->saveField("community",$this->request->data['level']-1);
        $response['result'] = $this->request->data['id'];
        echo json_encode($response);
    }
    ...
    public function view($id = null,$group_name = null,$invite_checksum = null) {
        ...
        $group['Group']['group_active_user_count'] = $this->GroupUser->getActiveUsers($id);
        if ($this->request->is('requested')) {
            $data = array(
                'groupAdmin' => array(),
                'groupAdminCnt' => 0,
