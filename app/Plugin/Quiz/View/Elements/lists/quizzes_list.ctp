        ...
        <div class="quiz_sort list_option">
            <div class="dropdown">
                ...
                <ul role="menu" class="dropdown-menu browseQuizzes" aria-labelledby="dropdown-sort">
                    <li class="no-current">
                        <a href="<?php echo (!empty($cat_id) ? $this->request->base . '/quizzes/index/' . $aCategory['Category']['id'] . '/' . seoUrl($aCategory['Category']['name']) : $this->request->base . '/quizzes'); ?>" data-url="<?php echo $this->request->base . '/quizzes/browse/' . (!empty($cat_id) ? 'category/' . $cat_id : (!empty($type) ? $type : 'all')) . '/latest'; ?>" rel="list-content"><?php echo __d('quiz', 'Latest'); ?></a>
                    </li>
                    <li class="no-current">
                        <a href="<?php echo (!empty($cat_id) ? $this->request->base . '/quizzes/index/' . $aCategory['Category']['id'] . '/' . seoUrl($aCategory['Category']['name']) : $this->request->base . '/quizzes'); ?>" data-url="<?php echo $this->request->base . '/quizzes/browse/' . (!empty($cat_id) ? 'category/' . $cat_id : (!empty($type) ? $type : 'all')) . '/taken'; ?>" rel="list-content"><?php echo __d('quiz', 'Taken'); ?></a>
                    </li>
                    <li class="no-current">
                        <a href="<?php echo (!empty($cat_id) ? $this->request->base . '/quizzes/index/' . $aCategory['Category']['id'] . '/' . seoUrl($aCategory['Category']['name']) : $this->request->base . '/quizzes'); ?>" data-url="<?php echo $this->request->base . '/quizzes/browse/' . (!empty($cat_id) ? 'category/' . $cat_id : (!empty($type) ? $type : 'all')) . '/like'; ?>" rel="list-content"><?php echo __d('quiz', 'Like'); ?></a>
                    </li>
                </ul>
            </div>
        </div>
                ...
                <div class="list_option">
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu" role="menu">
                            ...
                            <li><a href="javascript:void(0);" class="deleteQuiz" data-id="<?php echo $quiz['Quiz']['id']; ?>"><?php echo __d('quiz', 'Delete'); ?></a></li>
                        </ul>
                    </div>
                </div>
                <?php endif; ?>
                ...
                <div class="quiz-description-truncate">
                    ...
                    <div class="like-section">
                        <div class="like-action">
                            ...
                            <a data-type="Quiz_Quiz" data-id="<?php echo $quiz['Quiz']['id']; ?>" data-status="1" href="javascript:void(0)" class="likeItem">
                                <i class="material-icons">thumb_up</i>
                            </a>
                            <?php
                            $this->MooPopup->tag(array(
                                'href'=>$this->Html->url(array("controller" => "likes", "action" => "ajax_show", "plugin" => false, 'Quiz_Quiz', $quiz['Quiz']['id'])),
                                'title' => __d('quiz', 'People Who Like This'),
                                'innerHtml'=> '<span class="likeCount">' . $quiz['Quiz']['like_count'] . '</span>',
                            ));
                            ?>
                            
                            <?php if(empty($hide_dislike)): ?>
                            <a data-type="Quiz_Quiz" data-id="<?php echo $quiz['Quiz']['id']; ?>" data-status="0" href="javascript:void(0)" class="likeItem">
                                <i class="material-icons">thumb_down</i>
                            </a>
                            <?php
                            $this->MooPopup->tag(array(
                                'href'=>$this->Html->url(array("controller" => "likes", "action" => "ajax_show", "plugin" => false, 'Quiz_Quiz', $quiz['Quiz']['id'], 1 )),
                                'title' => __d('quiz', 'People Who DisLike This'),
                                'innerHtml'=>  '<span class="dislikeCount">' . $quiz['Quiz']['dislike_count'] . '</span>',
                            ));
                            ?>
                            <?php endif; ?>
