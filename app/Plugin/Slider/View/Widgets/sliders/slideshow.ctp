...
<?php endif;?>
<div class="front-demo" id="front-demo-<?php echo $key?>">
    <?php foreach($slides as $slide):?>

	<a href="<?php echo $slide['Slide']['link'];?>" class="slide1" data-lazy-background="<?php echo $slideHelper->getImage($slide, array())?>" <?php if($slide['Slide']['new_tab']):?>target="_blank"<?php endif;?>>
        <div class="bg_text_slide" <?php if($slider['Slider']['background_caption']):?> style="background-color: <?php echo $slider['Slider']['background_caption']?>" <?php endif;?>>
            ...
            <div style="color:<?php if($slide['Slide']['color']){echo $slide['Slide']['color'];}else{ echo '#000';}?>;font-size: <?php echo ($slide['Slide']['font_size']) ? $slide['Slide']['font_size'] : '12';?>px;"
                ...
                <?php echo nl2br(htmlspecialchars($slide['Slide']['text']));?>
            </div>
            <?php endif;?>
        </div>
	</a>
    ...
    <?php $this->Html->scriptStart(array('inline' => false, 'domReady' => true, 'requires'=>array('mooSlider', 'jquerydevramaslider'), 'object' => array('mooSlider'))); ?>
        mooSlider.initSlideshow({
            'key' : '<?php echo $key;?>',
