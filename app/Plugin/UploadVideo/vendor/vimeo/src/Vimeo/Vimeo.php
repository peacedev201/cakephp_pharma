...
class Vimeo
{
    ...
    public function upload($file_path, $upgrade_to_1080 = false, $machine_id = null)
    {
        ...
        if (!is_file($file_path)) {
            return array('moo_error' => 'Unable to locate file to upload.');
        }
        ...
        if ($ticket['status'] !== 201) {
            $error = !empty($ticket['body']['error']) ? $ticket['body']['error'] : $ticket['body'];
            return array('moo_error' => $error);
        }
        /* Integration */
    ...
    private function perform_upload($file_path, $ticket)
    {
        ...
        return array('vimeo_uri' => $completion['headers']['Location']);
    }
    ...
    public function getImageThumbnailVimeo($video_id = ""){
        if(empty($video_id)){
            return false;
        }

        $response = $this->request('/videos/' . $video_id .'/pictures');
