...
class EventListener implements CakeEventListener
{
    ...
    public function doSaveUser($event)
    {
    	...
    	if ($controller->Session->read('event_invite_checksum') && $controller->Session->read('event_invite_status'))
    	{
    		...
    		if (!empty($invite)) {
    			...
    			$eventRsvpModel->save(array(
    				'event_id' => $invite['EventUserInvite']['event_id'], 'user_id' => $controller->User->id, 'rsvp' => $controller->Session->read('event_invite_status')
    			));
