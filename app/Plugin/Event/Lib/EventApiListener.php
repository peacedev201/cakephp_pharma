...
class EventApiListener implements CakeEventListener {
    ...
    function getEvent($id, $v) {
        ...
        return array($event['Event']['id'], //id
            ...
            h($event['Event']['title']),
            $imageArray,
            $v->Time->event_format($event['Event']['from'], '%B %d, %Y') . $event['Event']['from_time'] . " - " . $v->Time->event_format($event['Event']['to'], '%B %d, %Y') . $event['Event']['to_time'],
    ...
    function exportEventAttend($e) {
        ...
        switch (count($events)):
            case 1:
                ...
                $attendingHtml = sprintf($attending1, $v->Html->link(h($events[0]['Event']['title']), FULL_BASE_URL . str_replace('?','',mb_convert_encoding($events[0]['Event']['moo_href'], 'UTF-8', 'UTF-8')) ));
                break;
            case 2:
                $attending = sprintf($attending2, h($events[0]['Event']['title']), h($events[1]['Event']['title']));
                $attendingHtml = sprintf($attending2, $v->Html->link(h($events[0]['Event']['title']), FULL_BASE_URL . str_replace('?','',mb_convert_encoding($events[0]['Event']['moo_href'], 'UTF-8', 'UTF-8'))), $v->Html->link(h($events[1]['Event']['title']), FULL_BASE_URL . str_replace('?','',mb_convert_encoding($events[1]['Event']['moo_href'], 'UTF-8', 'UTF-8'))));
                break;
            case 3:
            default :
                $attending = sprintf($attending2, h($events[0]['Event']['title']), abs(count($events) - 1) . ' ' . __('others'));
                $attendingHtml = sprintf($attending2, $v->Html->link(h($events[0]['Event']['title']), FULL_BASE_URL . str_replace('?','',mb_convert_encoding($events[0]['Event']['moo_href'], 'UTF-8', 'UTF-8'))), abs(count($events) - 1) . ' ' . __('others'));
                break;
        endswitch;
        ...
        foreach ($events as $event):
            ...
            $eventArray[] = array(
                ...
                'images' => $imageArray,
                'time' =>  $v->Time->event_format($event['Event']['from'], '%B %d, %Y') . $event['Event']['from_time'] . " - " . $v->Time->event_format($event['Event']['to'], '%B %d, %Y') . $event['Event']['to_time'],
