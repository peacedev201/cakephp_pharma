            ...
            <?php foreach ($popular_albums as $album): ?>
                <li>
                    ...
                    <a class="popular_album_cover" href="<?php echo $this->request->base?>/albums/view/<?php echo $album['Album']['id']?>/<?php echo seoUrl($album['Album']['moo_title'])?>">
                        <img src="<?php echo $photoHelper->getAlbumCover($album['Album']['cover'], array('prefix' => '150_square')) . '?' . time(); ?>" alt="<?php echo h($album['Album']['moo_title'])?>" />
                        <div class="gradient_bg"></div>
                        <div class="album-title">
                            <?php echo h($album['Album']['moo_title'])?>
                        </div>
                    </a>
                    </div>
                </li>
