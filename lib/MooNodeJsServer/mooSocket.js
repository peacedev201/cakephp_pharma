...
var chatSocket = (function () {
    ...
    var initFriendsAndBlocker = function(socket){
        mooDB.query(mooDB.mysql.format(mooDB.sqlString.getAllMyFriendId, [socket.userId]), function (err, rows) {
            if (err) {
            ...
            mooDB.query(mooDB.mysql.format(mooDB.sqlString.getMyBlockers, [socket.userId]), function (err, rows) {
                if (err) {
