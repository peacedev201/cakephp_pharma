...
var ChatUserWillBeRemoved = (function () {
    ...
    var updateHideMyOnlineStatus=function(id,isNotificationOffline){
        chatDB.query(chatDB.mysql.format(chatDB.sqlString.getHideOnlineSetting, [id]), function (err, result) {
            if (err) {
                log.error("getHideOnlineSetting error", err);
            } else {
                if(result.length > 0 ){
    ...
    var getMyFriends = function (socket,ids) { console.log("getMyFriends");
...
module.exports = ChatUserWillBeRemoved;
